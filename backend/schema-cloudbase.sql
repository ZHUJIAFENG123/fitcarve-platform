-- ============================================
-- 炼刻健身平台 - CloudBase SQL 型数据库建表脚本
-- 说明：在 CloudBase SQL 编辑器中直接执行
-- ============================================
...
CREATE TABLE IF NOT EXISTS courses (
  ...
  syllabus        TEXT            NULL                    COMMENT '课程大纲(JSON)',
...
CREATE TABLE IF NOT EXISTS versions (
  ...
  target_type VARCHAR(20)     NOT NULL                COMMENT '版本类型',
  target_id   INT             NOT NULL                COMMENT '目标ID',
  version     INT             NOT NULL                COMMENT '版本号',
  content     TEXT            NOT NULL                COMMENT '版本快照(JSON)',
...
CREATE TABLE IF NOT EXISTS training_plans (
  ...
  goal           VARCHAR(20)     NOT NULL                COMMENT '目标',
  level          VARCHAR(20)     NOT NULL                COMMENT '难度',
  duration_weeks INT             NOT NULL DEFAULT 4      COMMENT '训练周期(周)',
  days_per_week  INT             NOT NULL DEFAULT 4      COMMENT '每周训练天数',
  equipment      VARCHAR(255)    DEFAULT ''              COMMENT '所需器材',
  coach          VARCHAR(50)     NOT NULL                COMMENT '作者',
  author_id      INT             NULL                    COMMENT '创建者用户ID',
  views          INT             DEFAULT 0               COMMENT '浏览量',
  enrolled       INT             DEFAULT 0               COMMENT '参与人数',
  is_official    TINYINT(1)      NOT NULL DEFAULT 1      COMMENT '1=官方预置/0=用户创建',
  is_public      TINYINT(1)      NOT NULL DEFAULT 1      COMMENT '1=公开/0=私有',
  source         VARCHAR(20)     DEFAULT 'manual'        COMMENT '创建方式',
  syllabus        JSON            NOT NULL                COMMENT '训练大纲(JSON)',
  created_at      DATETIME        NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS exercises (
  ...
  muscle_group    VARCHAR(50)     NOT NULL                COMMENT '肌群',
  ...
  target_muscles    VARCHAR(255) DEFAULT ''              COMMENT '目标肌群',
  secondary_muscles VARCHAR(255) DEFAULT ''              COMMENT '辅助肌群',
...
CREATE TABLE IF NOT EXISTS diet_plans (
  ...
  meals           JSON            NOT NULL                COMMENT '餐食结构(JSON)',
...
CREATE TABLE IF NOT EXISTS foods (
  ...
  vitamin_a         DECIMAL(8,1) DEFAULT NULL            COMMENT '维生素A',
  vitamin_c         DECIMAL(8,1) DEFAULT NULL            COMMENT '维生素C',
  vitamin_e         DECIMAL(8,2) DEFAULT NULL            COMMENT '维生素E',
  cholesterol       DECIMAL(8,1) DEFAULT NULL            COMMENT '胆固醇',
  calcium           DECIMAL(8,1) DEFAULT NULL            COMMENT '钙',
  iron              DECIMAL(8,2) DEFAULT NULL            COMMENT '铁',
  sodium            DECIMAL(8,1) DEFAULT NULL            COMMENT '钠',
  potassium         DECIMAL(8,1) DEFAULT NULL            COMMENT '钾',
  phosphorus        DECIMAL(8,1) DEFAULT NULL            COMMENT '磷',
  magnesium         DECIMAL(8,1) DEFAULT NULL            COMMENT '镁',
  zinc              DECIMAL(8,2) DEFAULT NULL            COMMENT '锌',
  selenium          DECIMAL(8,1) DEFAULT NULL            COMMENT '硒',
...
CREATE TABLE IF NOT EXISTS water_logs (
  ...
  goal_ml     INT             NOT NULL DEFAULT 2000     COMMENT '目标饮水量(ml)',
  created_at  DATETIME        NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  PRIMARY KEY (id),
  INDEX idx_user_date (user_id, log_date)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='水饮记录表';
...
CREATE TABLE IF NOT EXISTS coaches (
  ...
  name            VARCHAR(50)     NOT NULL                COMMENT '教练名',
  specialty       VARCHAR(100)    NOT NULL DEFAULT ''     COMMENT '专长',
  bio             TEXT            NULL                    COMMENT '简介',
  avatar_url      VARCHAR(255)    NOT NULL DEFAULT ''     COMMENT '头像',
  certifications  TEXT            NULL                    COMMENT '资质证书(JSON数组)',
  experience_years INT            NOT NULL DEFAULT 0       COMMENT '经验年限',
  rating          DECIMAL(2,1)    NOT NULL DEFAULT 0.0     COMMENT '评分',
  students_count  INT             NOT NULL DEFAULT 0       COMMENT '学员数',
  created_at      DATETIME        NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at      DATETIME        DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (id),
  INDEX idx_user (user_id)
...
CREATE TABLE IF NOT EXISTS learning_paths (
  id              INT             NOT NULL AUTO_INCREMENT  COMMENT '路径ID',
  title           VARCHAR(200)    NOT NULL                COMMENT '路径标题',
  description     TEXT            NULL                    COMMENT '路径描述',
  cover_image     VARCHAR(255)    DEFAULT ''              COMMENT '封面图',
  difficulty      VARCHAR(20)     NOT NULL DEFAULT 'beginner' COMMENT '难度',
  ...
  author          VARCHAR(50)     NOT NULL                COMMENT '作者',
  enrolled        INT             NOT NULL DEFAULT 0       COMMENT '报名人数',
  rating          DECIMAL(2,1)    NOT NULL DEFAULT 0.0     COMMENT '评分',
  status          VARCHAR(20)     NOT NULL DEFAULT 'published' COMMENT '状态',
  lessons         JSON            NOT NULL                COMMENT '课时列表(JSON)',
...
CREATE TABLE IF NOT EXISTS knowledge_graph_edges (
  id              INT             NOT NULL AUTO_INCREMENT  COMMENT '边ID',
  source_id       INT             NOT NULL                COMMENT '源节点ID',
  target_id       INT             NOT NULL                COMMENT '目标节点ID',
  relation_type   VARCHAR(50)     NOT NULL                COMMENT '关系类型',
  strength        DECIMAL(3,2)    NOT NULL DEFAULT 1.00    COMMENT '关系强度(0.00~1.00)',
  PRIMARY KEY (id),
  INDEX idx_source (source_id),
  INDEX idx_target (target_id),
  UNIQUE KEY uk_source_target (source_id, target_id, relation_type),
  CONSTRAINT fk_edge_source FOREIGN KEY (source_id) REFERENCES knowledge_graph_nodes(id) ON DELETE CASCADE,
  CONSTRAINT fk_edge_target FOREIGN KEY (target_id) REFERENCES knowledge_graph_nodes(id) ON DELETE CASCADE
